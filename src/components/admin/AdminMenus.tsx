...
export const DEFAULT_MENUS_CONFIG: MenusConfig = {
  desktopColumns: 3,
  mobileColumns: 1,
  desktopGap: 'md',
  mobileGap: 'sm',
...
/**
 * Build-time fallback poster URLs from `src/assets/menus/`. Used only when
 * no images have been uploaded to the server via `/admin` → tab "Archivos".
 */
const BUILT_IN_PREVIEW_POSTERS: string[] = MENUS_POSTERS.map((p) => p.src);
 ...
 * to persist it to `site_config.menus_config`.
 */
const PreviewFrame = ({
  frameWidth,
  columns,
...
const AdminMenus = () => {
  ...
  const [draft, setDraft] = useState<MenusConfig>(DEFAULT_MENUS_CONFIG);
  ...
  useEffect(() => {
    if (siteConfig?.menus_config) {
      setDraft({ ...DEFAULT_MENUS_CONFIG, ...siteConfig.menus_config });
    }
  }, [siteConfig?.menus_config]);

  /** Detect unsaved changes vs. server state */
  const savedConfig = siteConfig?.menus_config ?? DEFAULT_MENUS_CONFIG;
