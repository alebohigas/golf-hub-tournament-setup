 ...
 *  - Persists settings server-side via `site_config.premios_config`.
 *
...
export const DEFAULT_PREMIOS_CONFIG: PremiosConfig = {
  desktopColumns: 3,
  mobileColumns: 1,
  desktopGap: 'md',
  mobileGap: 'sm',
...
/**
 * Build-time fallback poster URLs from `src/assets/premios/`. Used only when
 * no images have been uploaded to the server via `/admin` → tab "Archivos".
 */
const BUILT_IN_PREVIEW_POSTERS: string[] = PREMIOS_POSTERS.map((p) => p.src);
 ...
 * to persist it to `site_config.premios_config`.
 */
const PreviewFrame = ({
  frameWidth,
  columns,
              ...
              >
                ...
                >
                  <div
                    ...
                    aria-label="Arrastra para reordenar este premio"
                  >
...
const AdminPremios = () => {
  ...
  const [draft, setDraft] = useState<PremiosConfig>(DEFAULT_PREMIOS_CONFIG);
  ...
  useEffect(() => {
    if (siteConfig?.premios_config) {
      setDraft({ ...DEFAULT_PREMIOS_CONFIG, ...siteConfig.premios_config });
    }
  }, [siteConfig?.premios_config]);

  /** Detect unsaved changes vs. server state */
  const savedConfig = siteConfig?.premios_config ?? DEFAULT_PREMIOS_CONFIG;
      ...
      },
      {
        onSuccess: () => {
          toast({
            ...
            description: `Premios: Desktop ${draft.desktopColumns} col / Mobile ${draft.mobileColumns} col.`,
          });
        },
        onError: (err) => {
          toast({
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Define la cantidad de columnas y el espaciado del grid de premios.
