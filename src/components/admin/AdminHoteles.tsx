...
export const DEFAULT_HOTELES_CONFIG: HotelesConfig = {
  desktopColumns: 3,
  mobileColumns: 1,
  desktopGap: 'md',
  mobileGap: 'sm',
 ...
 * to persist it to `site_config.hoteles_config`.
 */
const PreviewFrame = ({
  frameWidth,
  columns,
              ...
              >
                ...
                >
                  <div
                    ...
                    aria-label="Arrastra para reordenar este hotel"
                  >
...
const AdminHoteles = () => {
  ...
  const [draft, setDraft] = useState<HotelesConfig>(DEFAULT_HOTELES_CONFIG);
  ...
  useEffect(() => {
    if (siteConfig?.hoteles_config) {
      setDraft({ ...DEFAULT_HOTELES_CONFIG, ...siteConfig.hoteles_config });
    }
  }, [siteConfig?.hoteles_config]);

  /** Detect unsaved changes vs. server state */
  const savedConfig = siteConfig?.hoteles_config ?? DEFAULT_HOTELES_CONFIG;
      ...
      },
      {
        onSuccess: () => {
          toast({
            ...
            description: `Hoteles: Desktop ${draft.desktopColumns} col / Mobile ${draft.mobileColumns} col.`,
          });
        },
        onError: (err) => {
          toast({
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Define la cantidad de columnas y el espaciado del grid de hoteles.
