 ...
 * Each card displays a poster imported from `src/assets/eventos` or uploaded
...
const AtraccionesSection = () => {
   ...
   */
  useEffect(() => {
    if (openIndex === null || orderedAtracciones.length < 2) return;
    const neighbours = [
      orderedAtracciones[(openIndex + 1) % orderedAtracciones.length],
      orderedAtracciones[(openIndex - 1 + orderedAtracciones.length) % orderedAtracciones.length],
    ];
