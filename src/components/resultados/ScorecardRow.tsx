...
const getScoreColor = (golpes: number, par: number): string => {
  ...
  if (diff === -1) return 'bg-[#900000]/20 text-[#900000]';             // Birdie
              ...
              {holes.map(h => {
                ...
                return (
                  <td key={h.hoyo} className={`px-2 py-1 text-center font-medium ${
                    ...
                    diff < 0 ? 'text-[#900000]' : diff > 0 ? 'text-blue-600' : 'text-muted-foreground'
                  }`}>
            ...
            {type === 'scratch' && (
              <span className="text-muted-foreground">
                {(() => {
                  ...
                  const cls = d < 0 ? 'text-[#900000]' : d > 0 ? 'text-blue-600' : '';
